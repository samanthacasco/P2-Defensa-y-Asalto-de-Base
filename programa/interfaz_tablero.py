...
from economia import comprar_unidad, comprar_torre, comprar_muro, recompensa_atacante, recompensa_defensor
...
def mover_arriba_interfaz(ventana, partida):
    ...
    if ventana.objeto_seleccionado is not None and isinstance(ventana.objeto_seleccionado, Unidad):
        ...
        # Limpiamos la selección de la casilla para que no se quede viendo ahí
...
def comprar_soldado_interfaz(ventana, partida):
    ...
    if compra_exitosa:
        # Quitamos la selección visual de la casilla y el objeto
        ventana.fila_seleccionada = None
        ventana.columna_seleccionada = None
...
def atacar_interfaz(ventana, partida):
    ...
    # Si el atacante dañó una torre o la base, gana dinero según el daño realizado
    if isinstance(ventana.objetivo_seleccionado, (Torre, Base)):
        recompensa_atacante(partida, dano_realizado)
...
def combate_automatico(partida):
    """Ejecuta toda la fase de combate automáticamente, usando las habilidades.
    Primero las torres atacan a las unidades en su alcance (y el defensor gana dinero
    por cada unidad eliminada); luego las unidades atacan a las torres, muros o la base
    en su alcance (y el atacante gana dinero según el daño realizado).
    Recibe la partida.
    Devuelve una lista de textos describiendo lo que ocurrió.
    """
    eventos = []   # textos para el resumen visual

    # 1) las torres atacan a las unidades en su alcance, usando su habilidad
    for torre in list(partida.mapa.torres):
        for unidad in list(partida.mapa.unidades):
            if esta_al_alcance(torre, unidad):
                ...
                resultado, habilidad = ejecutar_ataque_con_habilidad(torre, unidad, partida.mapa)
                dano = vida_antes - unidad.vida

                if unidad.esta_destruida():
                    # el defensor gana dinero por eliminar una unidad
                    recompensa_defensor(partida, 1)
                    texto = f"Torre {torre.nombre} eliminó a {unidad.nombre} ({dano} de daño)"
                else:
                    texto = f"Torre {torre.nombre} dañó a {unidad.nombre}: {dano} (le queda {unidad.vida})"

                # agrega el nombre de la habilidad si se activó
    ...
    # 2) las unidades atacan a las torres, muros o la base en su alcance, usando su habilidad
    for unidad in list(partida.mapa.unidades):
        objetivo = buscar_objetivo_defensor(unidad, partida)
        if objetivo is not None:
            vida_antes = objetivo.vida
            resultado, habilidad = ejecutar_ataque_con_habilidad(unidad, objetivo, partida.mapa)
            dano = vida_antes - objetivo.vida

            # el atacante gana dinero según el daño que realizó
            recompensa_atacante(partida, dano)
            ...
            if objetivo.esta_destruida() and not isinstance(objetivo, Base):
                texto = f"{unidad.nombre} destruyó {nombre_obj} ({dano} de daño)"
            else:
                texto = f"{unidad.nombre} atacó {nombre_obj}: {dano} de daño"
...
def mostrar_instrucciones_inicio(partida):
    ...
    texto = ("Bienvenido a Defensa y Asalto de Base\n\n"
             ...
             "- Las torres atacan solas al presionar 'Iniciar combate'.\n\n"
             "La guia de cada turno aparece en el panel izquierdo.")
...
def mostrar_tablero(ventana, partida):
    ...
    if not tablero_iniciado:
        ...
        # Conectamos dinámicamente los costos reales del modelo.py
        tk.Button(frame_atacante, text=f"Soldado (${Soldado().costo})", width=18, command=lambda: comprar_soldado_interfaz(ventana, partida)).grid(row=0, column=0, pady=3)
        tk.Button(frame_atacante, text=f"Tanque (${Tanque().costo})", width=18, command=lambda: comprar_tanque_interfaz(ventana, partida)).grid(row=1, column=0, pady=3)
        tk.Button(frame_atacante, text=f"Unidad Rápida (${UnidadRapida().costo})", width=18, command=lambda: comprar_unidad_rapida_interfaz(ventana, partida)).grid(row=2, column=0, pady=3)
        ...
        # Conectamos dinámicamente los costos reales del modelo.py
        tk.Button(frame_defensor, text=f"Comprar Muro (${Muro().costo})", width=18, command=lambda: comprar_muro_interfaz(ventana, partida)).grid(row=0, column=0, pady=3)
        tk.Button(frame_defensor, text=f"Torre Básica (${TorreBasica().costo})", width=18, command=lambda: comprar_torre_basica_interfaz(ventana, partida)).grid(row=1, column=0, pady=3)
        tk.Button(frame_defensor, text=f"Torre Pesada (${TorrePesada().costo})", width=18, command=lambda: comprar_torre_pesada_interfaz(ventana, partida)).grid(row=2, column=0, pady=3)
        tk.Button(frame_defensor, text=f"Torre Mágica (${TorreMagica().costo})", width=18, command=lambda: comprar_torre_magica_interfaz(ventana, partida)).grid(row=3, column=0, pady=3)
...
def actualizar_casillas(ventana, partida, tamano):
    ...
    Resalta de forma sutil la casilla seleccionada.
    ...
    for fila in range(partida.mapa.filas):
        for columna in range(partida.mapa.columnas):
            ...
            # resalta de forma sutil la casilla seleccionada (mismo estilo, borde un poco marcado)
            if fila == ventana.fila_seleccionada and columna == ventana.columna_seleccionada:
                ventana.botones[fila][columna].config(bg="#ffcccc", relief="solid", bd=2)
            else:
